...
public class LinkedList<E> implements List<E> {
	...
	public int replaceAll(E e, E f) {
		int count = 0;
		for(Node node = this.header.next;node!=null;node=node.next) { // Start at the element following the header until the last
	...
	public List<E> reverse() {
		...
		for(int i = this.size()-1;i>=0;i--) { // Start at the last element and iterate towards the first
			list.add(this.get(i));
		}
		return list;
	}
