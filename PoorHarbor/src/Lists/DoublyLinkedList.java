...
public class DoublyLinkedList<E> implements List<E> {
	...
	/* private fields */
	public Node header, trailer; // "dummy" nodes
	...
	public void add(E obj) {
		...
		/* TODONE With a Doubly Linked List (with header AND trailer), this is easy.
	...
	public void add(int index, E obj) {
		...
		if (index == size())
			...
			// TODONE For a DLL, what else needs to be done? Try a diagram; consider edge cases.
	...
	public boolean remove(E obj) {
		...
		if (nextNode != trailer) { // Found it!
			...
			// TODONE For a DLL, what else needs to be done? See comment above for a hint.
	...
	public boolean remove(int index) {
		...
		Node prevNode;
		Node nextNode;
		// TODONE These variables could be helpful: Node prevNode, nextNode;
		...
		prevNode = rmNode.prev;
		nextNode = rmNode.next;
		
		// TODONE For a DLL, what needs to be done?
	...
	public int replaceAll(E e, E f) {
		int count = 0;
		for(Node node = this.header.next;node!=trailer;node=node.next) { // Start at the element following the header until the last
			if(node.value.equals(e)) { // Check similarity
				node.value = f; // Replace value
				count++;
			}
		}
		return count;
